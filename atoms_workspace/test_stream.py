...
root = Agent(name='a', model='gemini-3.5-flash-lite',
             instruction='say hello to the world in 5 sentences')
...
async def main():
    ...
    async for e in runner.run_async(user_id='u1', session_id=session.id, new_message=content, run_config=run_config):
        print(
            f"EVENT: partial={e.partial}, text={repr(e.text if hasattr(e, 'text') else None)}")
